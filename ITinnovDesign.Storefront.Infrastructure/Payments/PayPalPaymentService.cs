...
namespace ITinnovDesign.Storefront.Infrastructure.Payments
{
    ...
    {
        public PaymentPostData GeneratePostDataFor(OrderPaymentRequest orderRequest, HttpContext ctx)
        {
            ...
            postDataAndValue.Add("return",
                         Helpers.UrlHelper.Resolve("/Payment/PaymentComplete", ctx));
            ...
            postDataAndValue.Add("cancel_return",
                         Helpers.UrlHelper.Resolve("/Payment/PaymentCancel", ctx));
            ...
            postDataAndValue.Add("notify_url",
                        Helpers.UrlHelper.Resolve("/Payment/PaymentCallBack", ctx));
                                                ...
                                                FormCollection collection, HttpContext ctx)
        {
            ...
            string response = ValidatePaymentNotification(collection, ctx);
        ...
        private string ValidatePaymentNotification(IFormCollection formCollection, HttpContext ctx)
        {

            formCollection.Append(new KeyValuePair<string, StringValues>("cmd", "_notify - validate"));
            ...
            byte[] param = null;
            using (var ms = new MemoryStream(Convert.ToInt32(ctx.Request.ContentLength)))
            {
                 ctx.Request.Body.CopyToAsync(ms).Wait();
                  param = ms.ToArray();
            }
